...
grep -F "fork_patches=$repo_root/fork_patches/patches/0001-build-config-add-loong64-sysroot.patch $repo_root/fork_patches/patches/0002-build-config-skip-loong64-clang-builtins.patch" <<<"$output" >/dev/null || fail "missing fork patches"
...
grep -F "ref: \${{ github.ref_name }}" "$workflow" >/dev/null || fail "missing fork branch checkout"
